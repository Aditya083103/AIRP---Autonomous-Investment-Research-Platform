...
| `run_eval_debate.py` | Run the debate-quality LangSmith eval (T-070) against the 5 synthetic post-debate snapshots in `backend/evals/debate_eval_dataset.py` — fully deterministic, no network or LLM call needed for grading; prints a per-snapshot pass/fail report across all 4 rubric checks (Contrarian disagreement, multi-agent engagement, novelty, PM engagement), and (when `LANGSMITH_API_KEY` is set) uploads the dataset and runs a real LangSmith experiment |