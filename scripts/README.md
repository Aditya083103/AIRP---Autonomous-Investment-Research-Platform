...
| `run_eval_latency.py` | Run the real end-to-end pipeline for 3 companies (T-071) — times each run, captures the per-node latency breakdown via a log-capture handler, and prints a p50/p95-vs-target report with the identified bottleneck node |